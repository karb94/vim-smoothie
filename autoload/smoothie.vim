...
function s:scroll_up()
    if line('.') > 1
        call s:execute_preserving_scroll("normal! 1\<C-E>")
        return 0
    else
        return 1
    endif
...
function s:scroll_down()
    if line('.') < line('$')
        call s:execute_preserving_scroll("normal! 1\<C-Y>")
        return 0
    else
        return 1
    endif
...
""
" Perform as many scrolls up or down to move {lines} lines from the starting
" position (negative {lines} value means to go up).  Return 1 if hit either
" top or bottom, and cannot move further.
function s:scroll_many(lines)
    let l:remaining_lines = a:lines
    while 1
        if l:remaining_lines < 0
            if s:scroll_up()
...
" Start the animation timer if not already running.  Should be called when
" updating the target, when there's a chance we're not already moving.
function s:start_scrolling()
    if !exists('s:timer_id')
        let s:timer_id = timer_start(g:smoothie_update_interval, function("s:scrolling_tick"), {'repeat': -1})
    endif
endfunction
...
" position).  If we're already moving, try to do the smart thing, taking into
" account our progress in reaching the target set previously.
function s:update_target_scroll(lines)
    if g:smoothie_break_on_reverse && s:target_displacement * a:lines < 0
        call s:stop_moving()
    else
        let s:target_displacement += a:lines
        call s:start_scrolling()
    endif
endfunction
...
function smoothie#scrollup()
    call s:count_to_scroll()
    call s:update_target_scroll(float2nr(0.10*winheight(0)))
endfunction

""
" Smooth equivalent to ^Y.
function smoothie#scrolldown()
    call s:count_to_scroll()
    call s:update_target_scroll(-float2nr(0.10*winheight(0)))
endfunction
""
